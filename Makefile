...
list-tests:
	@ $(HEADER) "=====> Unit tests..."
	@ echo "make unit"
	@ cat ./corefunc/*_test.go | ggrep "func Test" | $(SED) 's/func\s//g' | $(SED) -r 's/\(.*//g' | $(SED) -r 's/Test/make unit NAME=/g'

	@ $(HEADER) "=====> Terraform acceptance tests..."
	@ echo "make acc"
	@ cat ./corefuncprovider/*_test.go | ggrep "func TestAcc" | $(SED) 's/func\s//g' | $(SED) -r 's/\(.*//g' | $(SED) -r 's/TestAcc/make acc NAME=/g'

	@ $(HEADER) "=====> Example tests..."
	@ echo "make examples"
	@ cat ./corefunc/*_test.go | ggrep "func Example" | $(SED) 's/func\s//g' | $(SED) -r 's/\(.*//g' | $(SED) -r 's/Example/make examples NAME=/g'
