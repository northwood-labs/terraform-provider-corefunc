...
func TestAccTimeParseFunction(t *testing.T) {
	...
	for name, tc := range testfixtures.TimeParseTestTable { // lint:no_dupe
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.New("time_parse_function_fixture.tftpl").
				Funcs(FuncMap()).
				ParseFiles("time_parse_function_fixture.tftpl"),
		)
			...
			Steps: []resource.TestStep{
				{
					...
					ConfigStateChecks: []statecheck.StateCheck{
						statecheck.ExpectKnownOutputValue("time_parse", knownvalue.Int64Exact(tc.Expected)),
					},
