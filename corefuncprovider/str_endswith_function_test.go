...
func TestAccStrEndswithFunction(t *testing.T) {
	t.Parallel()
	...
	for name, tc := range testfixtures.StrEndsWithTestTable { // lint:no_dupe
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.New("str_endswith_function_fixture.tftpl").
				Funcs(FuncMap()).
				ParseFiles("str_endswith_function_fixture.tftpl"),
		)
			...
			Steps: []resource.TestStep{
				{
					...
					ConfigStateChecks: []statecheck.StateCheck{
						statecheck.ExpectKnownOutputValue("str_endswith", knownvalue.Bool(tc.Expected)),
					},
