...
func TestAccHashSha1Base64Function(t *testing.T) {
	t.Parallel()
	...
	for name, tc := range testfixtures.Base64HashSHA1TestTable { // lint:no_dupe
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.New("hash_sha1_base64_function_fixture.tftpl").
				Funcs(FuncMap()).
				ParseFiles("hash_sha1_base64_function_fixture.tftpl"),
		)
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						resource.TestCheckOutput("sha1_base64", tc.Expected),
					),
