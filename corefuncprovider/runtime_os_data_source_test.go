...
func TestAccRuntimeOsDataSource(t *testing.T) {
	...
	for name, tc := range testfixtures.RuntimeOsTestTable { // lint:no_dupe
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.ParseFiles("runtime_os_data_source_fixture.tftpl"),
		)
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.corefunc_runtime_os.os", "value", tc.Expected),
					),
				},
			},
		})
