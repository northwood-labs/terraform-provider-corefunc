...
func TestAccRuntimeCpuarchDataSource(t *testing.T) {
	...
	for name, tc := range testfixtures.RuntimeCpuarchTestTable { // lint:no_dupe
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.ParseFiles("runtime_cpuarch_data_source_fixture.tftpl"),
		)
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.corefunc_runtime_cpuarch.arch", "value", tc.Expected),
					),
				},
			},
		})
