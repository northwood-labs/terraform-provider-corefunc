...
func TestAccHashArgon2idBase64Function(t *testing.T) {
	t.Parallel()
	...
	for name, tc := range testfixtures.Base64HashArgon2idTestTable { // lint:no_dupe
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.New("hash_argon2id_base64_function_fixture.tftpl").
				Funcs(FuncMap()).
				ParseFiles("hash_argon2id_base64_function_fixture.tftpl"),
		)
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						resource.TestCheckOutput("hash_argon2id_base64", tc.Expected),
					),
