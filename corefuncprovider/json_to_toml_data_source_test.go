...
func TestAccJSONToTomlDataSource(t *testing.T) {
	t.Parallel()

	funcName := traceFuncName()

	for name, tc := range testfixtures.JSONtoTOMLTestTable { // lint:no_dupe
		fmt.Printf(
			"=== RUN   %s/%s\n",
		...
		buf := &bytes.Buffer{}
		tmpl := template.Must(
			template.ParseFiles("json_to_toml_data_source_fixture.tftpl"),
		)
